...
lam_template = lam[:11]  # first 11 bands match filt_files (no WISE)
...
def UVExcess(index):
    if ((((f_ng[index] > 0 and f_gr[index] < 0) or (f_fn[index] > 0 and f_ng[index] < 0)) and (flam_fuv[index] > flam_nuv[index])) and ebv[index] > 0.2): 
        return True
    return False
...
for index, name in enumerate(targetID):
    ...
    if UVExcess(index):
        ...
        obs_flam = np.array([
            ...
            flam_Y.value[index],   flam_J.value[index],
            flam_H.value[index],   flam_K.value[index],
        ])
