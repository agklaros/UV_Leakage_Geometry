"""Batch PDF report of every UV-excess candidate SED with its review status.

Steps through each candidate in data/matched/uv_excess_candidates_w2m.csv (same
flux conversion / template-scaling logic as review_uv_excess_sample.py), draws
one page per candidate into a single multi-page PDF, and annotates each page
with its Accept/Reject outcome from UV_EXCESS_SAMPLE_progress.csv.

Run: python scripts/seds/uv_excess_review_report.py
"""
import warnings
...
REPORT_PDF = BASE_DIR / "data/matched/uv_excess_review_report.pdf"
...
def annotate_status(ax, decision):
    ...
    ax.text(
        0.98, 0.95, label, transform=ax.transAxes,
...
def main():
    ...
    counts = {'YES': 0, 'NO': 0, 'unreviewed': 0}

    with PdfPages(REPORT_PDF) as pdf:
        for n, index in enumerate(candidates, start=1):
            plot_sed(ax, index)
            decision = decisions.get(target_name(index))
            annotate_status(ax, decision)
            if decision == 'YES':
                counts['YES'] += 1
            elif decision == 'NO':
                counts['NO'] += 1
                ...
                counts['unreviewed'] += 1
            fig.suptitle(f"Candidate {n} / {len(candidates)}")
            pdf.savefig(fig)

    plt.close(fig)

    print(
        f"wrote {REPORT_PDF} ({len(candidates)} candidates: "
        f"{counts['YES']} accepted, {counts['NO']} rejected, "
        f"{counts['unreviewed']} not yet reviewed)"
    )
